...
spec = matrix(c(
  ...
  'input', 'i', 1, "character", "input the abuance of tax in each sample ,required",
  'outdir', 'o', 1, "character", "output file outdir,[optional, default: cwd]",
  'title', 't', 1, "character", "legend title [optional, default: demo]",
  'xlab', 'a', 1, "character", "xlab [optional, default: sample]",
  'ylab', 'b', 1, "character", "ylab [optional, default: Relative abundance]",
  'name', 'n', 1, "character", "out file name prefix[optional, default: demo]",
  'height', 'H', 1, "integer", "the height of graph ,unit is inches [optional, default: 5]",
  'width', 'W', 1, "integer", "the width of graph ,unit is inches [optional, default: 10]"
), byrow = TRUE, ncol = 5);
opt = getopt(spec);
...
if (is.null(opt$input)) { print_usage(spec) }
if (is.null(opt$outdir)) { opt$outdir = getwd() }
if (is.null(opt$title)) { opt$title = "demo" }
if (is.null(opt$xlab)) { opt$xlab = "sample" }
if (is.null(opt$ylab)) { opt$ylab = "Relative abundance" }
if (is.null(opt$name)) { opt$name = "demo" }
if (is.null(opt$height)) { opt$height = 5 }
if (is.null(opt$width)) { opt$width = 10 }
#set some reasonable defaults for the options that are needed,
#but were not specified.
if (!file.exists(opt$outdir)) {
  if (!dir.create(opt$outdir, showWarnings = FALSE, recursive = TRUE)) {
